...
const FisaDePostGenerator = () => {
    ...
    const handleGenerate = async () => {
        if (!company.address) {
            alert('Vă rugăm să completați toate informațiile companiei (nume, CUI/CIF, adresă)');
            return;
        }

        const validEmployees = employees.filter(emp =>
            emp.name.trim() && emp.documentDate.trim()
        );

        if (validEmployees.length === 0) {
            alert('Vă rugăm să adăugați cel puțin un angajat cu nume și dată document');
            return;
        }
        ...
        try {
            for (const employee of validEmployees) {
                const fisaData = {
                    ...
                    // Company information formatted as required
                    sc_cui_address: `${company.address}`,
                };

                await generateSingleFisa(fisaData);
            ...
            alert('Eroare la generarea fișelor de post');
        } finally {
            setLoading(false);
        }
    };

    const generateSingleFisa = async (data: any) => {
        try {
            const response = await fetch('/api/generate-fisa-de-post', {
                method: 'POST',
                headers: {
            ...
            link.href = url;
            link.download = `Fisa_de_Post_${data.employee_name.replace(/\s+/g, '_')}.docx`;
        ...
        <>
            <header className="mb-6">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-800">Generator Fișe de Post</h1>
                        <p className="text-gray-600 text-sm">Generați fișe de post pentru curieri</p>
                    </div>
                </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            placeholder="Ex: Str. Exemplu nr. 1, Sector 1, București"
                                            rows={3}
                                        />
                                        <p className="text-xs text-gray-500 mt-1">
                                            Adresa completă care va apărea în document
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-white rounded border border-gray-300 p-4">
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-200">
                            ...
                            <div className="text-xs text-gray-600 space-y-1">
                                <p>Șablon: <code className="bg-gray-100 px-1.5 py-0.5 rounded">Fisa_de_post_curier_WITH_PLACEHOLDERS.docx</code></p>
                                <p>Câmpurile marcate cu * sunt obligatorii</p>
                                <p>Fiecare angajat primește fișă de post separată</p>
...
export default FisaDePostGenerator