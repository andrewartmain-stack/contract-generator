...
const CerereConcediuDeOdihnaGenerator = () => {
    ...
    const handleGenerate = async () => {
        ...
        const validEmployees = employees.filter(emp =>
            emp.name.trim() &&
            emp.vacationPeriodStart.trim() &&
            emp.vacationPeriodEnd.trim() &&
            emp.documentDate.trim()
        );

        if (validEmployees.length === 0) {
            alert('Vă rugăm să adăugați cel puțin un angajat cu nume, perioada concediului și data documentului');
            return;
        }
        ...
        try {
            for (const employee of validEmployees) {
                // Форматируем период отпуска
                const vacationPeriod = `${formatDate(employee.vacationPeriodStart)} - ${formatDate(employee.vacationPeriodEnd)}`;
                ...
                const cerereData = {
                    ...
                    // Director name (если нужно, можно добавить в форму)
            ...
            alert('Eroare la generarea cererilor de concediu');
        } finally {
            setLoading(false);
        }
    };
    ...
    const generateSingleCerere = async (data: any) => {
        try {
            const response = await fetch('/api/generate-cerere-concediu', {
                method: 'POST',
                headers: {
            ...
            link.href = url;
            link.download = `Cerere_Concediu_${data.employee_name.replace(/\s+/g, '_')}.docx`;
            ...
            console.error('Error generating cerere concediu:', error);
            throw error;
        }
    };
        ...
        <>
            <header className="mb-6">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-800">Generator Cereri Concediu</h1>
                        <p className="text-gray-600 text-sm">Generați cereri de concediu de odihnă pentru angajați</p>
                    </div>
                </div>
                                ...
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 mt-1">
                                            Numele companiei care va apărea în cerere
                                        </p>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div key={employee.id} className="border border-gray-200 rounded p-4">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div className="space-y-3">
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-1">
                                                        Data începere concediu *
                                                    </label>
                                                    <input
                                                        type="date"
                                                        value={employee.vacationPeriodStart}
                                                        onChange={(e) => updateEmployee(employee.id, 'vacationPeriodStart', e.target.value)}
                                                        className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-gray-500 focus:border-transparent text-sm"
                                                    />
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    <input
                                                        type="date"
                                                        value={employee.vacationPeriodEnd}
                                                        onChange={(e) => updateEmployee(employee.id, 'vacationPeriodEnd', e.target.value)}
                                                        className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-gray-500 focus:border-transparent text-sm"
                                                    />
                                                </div>
                                                ...
                                                <div className="p-3 bg-gray-50 rounded border border-gray-200">
                                                    ...
                                                    {employee.vacationPeriodStart && employee.vacationPeriodEnd ? (
                                                        <p className="text-sm text-gray-600">
                                                            {formatDate(employee.vacationPeriodStart)} - {formatDate(employee.vacationPeriodEnd)}
                                                        </p>
                                                    ) : (
                                                        <p className="text-sm text-gray-400">
                                                            Selectați ambele date pentru a vedea perioada
                                                        </p>
                                                    )}
                                                </div>
                                            </div>
                    ...
                    <div className="bg-white rounded border border-gray-300 p-4">
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-200">
                            ...
                            <div className="text-xs text-gray-600 space-y-1">
                                <p>Șablon: <code className="bg-gray-100 px-1.5 py-0.5 rounded">cerere_concediu_de_odihna_model_MXA_WITH_PLACEHOLDERS.docx</code></p>
...
export default CerereConcediuDeOdihnaGenerator