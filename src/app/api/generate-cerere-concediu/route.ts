...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Generating cerere concediu for:', data.employee_name);
    ...
    const templatePath = path.join(
      ...
      'public',
      'templates',
      'cerere_concediu_de_odihna_model_MXA_WITH_PLACEHOLDERS.docx'
    );
    ...
    if (!fs.existsSync(templatePath)) {
      ...
      return NextResponse.json(
        {
          error:
            'Template file not found. Please upload cerere_concediu_de_odihna_model_MXA_WITH_PLACEHOLDERS.docx to public/templates/',
        },
        { status: 500 }
      );
    }
    ...
    const safeName = (data.employee_name || 'Cerere_Concediu')
    ...
    return new NextResponse(buffer as BodyInit | null | undefined, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Cerere_Concediu_${safeName}.docx"`,
    ...
    return NextResponse.json(
      {
        error: 'Failed to generate cerere concediu',
        details: error.message || 'Unknown error',
