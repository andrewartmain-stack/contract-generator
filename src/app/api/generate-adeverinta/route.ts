import { NextRequest, NextResponse } from 'next/server';
import Docxtemplater from 'docxtemplater';
import PizZip from 'pizzip';
import fs from 'fs';
import path from 'path';

export async function POST(request: NextRequest) {
  try {
    // Parse request data
    const data = await request.json();

    console.log('Generating adeverinta for:', data.employee_name);

    // Path to the template file
    const templatePath = path.join(
      process.cwd(),
      'public',
      'templates',
      'adeverinta_WITH_PLACEHOLDERS.docx'
    );

    // Check if template exists
    if (!fs.existsSync(templatePath)) {
      console.error('Template not found at:', templatePath);
      return NextResponse.json(
        {
          error:
            'Template file not found. Please upload adeverinta_WITH_PLACEHOLDERS.docx to public/templates/',
        },
        { status: 500 }
      );
    }

    // Read the template
    const content = fs.readFileSync(templatePath, 'binary');

    // Create PizZip instance
    const zip = new PizZip(content);

    // Initialize docxtemplater
    const doc = new Docxtemplater(zip, {
      paragraphLoop: true,
      linebreaks: true,
      delimiters: {
        start: '{{',
        end: '}}',
      },
    });

    // Prepare template data with all placeholders from the document
    const templateData = {
      // Company information
      company_name: data.company_name || '',
      company_city: data.company_city || '',
      company_sector: data.company_sector || '',
      company_street: data.company_street || '',
      company_number: data.company_number || '',
      company_registration_number: data.company_registration_number || '',
      company_fiscal_code: data.company_fiscal_code || '',
      representative_gender: data.representative_gender || '',
      company_representative: data.company_representative || '',
      company_representative_title: data.company_representative_title || '',

      // Employee information
      employee_name: data.employee_name || '',
      employee_country: data.employee_country || '',
      employee_birth_date: data.employee_birth_date || '',
      employee_passport_series: data.employee_passport_series || '',
      employee_passport_number: data.employee_passport_number || '',
      employee_passport_issued_by: data.employee_passport_issued_by || '',
      employee_work_permit: data.employee_work_permit || '',
      employee_work_permit_date: data.employee_work_permit_date || '',
      employee_service_address: data.employee_service_address || '',

      // Document date
      document_date: data.document_date || '',
    };

    console.log('Template data prepared with placeholders');

    // Set the template variables
    doc.setData(templateData);

    // Try to render the document
    try {
      doc.render();
    } catch (error: any) {
      console.error('Template rendering error:', error);

      // Log detailed error information
      if (error.properties && error.properties.errors) {
        error.properties.errors.forEach((err: any) => {
          console.error('Error details:', err);
        });
      }

      return NextResponse.json(
        {
          error: 'Template rendering failed',
          details: error.message || 'Unknown error',
          properties: error.properties,
        },
        { status: 400 }
      );
    }

    // Generate the output
    const buffer = doc.getZip().generate({
      type: 'nodebuffer',
      compression: 'DEFLATE',
    });

    // Create a safe filename
    const safeName = (data.employee_name || 'Adeverinta')
      .replace(/[^a-zA-Z0-9ăâîșțĂÂÎȘȚ ]/g, '')
      .replace(/\s+/g, '_');

    // Return the document
    return new NextResponse(buffer as BodyInit | null | undefined, {
      status: 200,
      headers: {
        'Content-Type':
          'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
        'Content-Disposition': `attachment; filename="Adeverinta_${safeName}.docx"`,
        'Cache-Control': 'no-cache',
      },
    });
  } catch (error: any) {
    console.error('Error generating adeverinta:', error);

    // Log stack trace for debugging
    if (error.stack) {
      console.error('Stack trace:', error.stack);
    }

    return NextResponse.json(
      {
        error: 'Failed to generate adeverinta',
        details: error.message || 'Unknown error',
        stack: process.env.NODE_ENV === 'development' ? error.stack : undefined,
      },
      { status: 500 }
    );
  }
}

// GET method to check if template exists
export async function GET() {
  try {
    const templatePath = path.join(
      process.cwd(),
      'public',
      'templates',
      'adeverinta_WITH_PLACEHOLDERS.docx'
    );
    const exists = fs.existsSync(templatePath);

    if (exists) {
      const stats = fs.statSync(templatePath);
      return NextResponse.json({
        status: 'ready',
        templateName: 'adeverinta_WITH_PLACEHOLDERS.docx',
        templateSize: `${(stats.size / 1024).toFixed(2)} KB`,
        lastModified: stats.mtime,
        message: 'Template found and ready for use',
        note: 'All placeholders are optional - unfilled placeholders will be empty',
        requiredPlaceholders: [
          'company_name',
          'company_fiscal_code',
          'company_representative',
          'employee_name',
          'employee_country',
          'document_date',
        ],
        allPlaceholders: [
          'company_name',
          'company_city',
          'company_sector',
          'company_street',
          'company_number',
          'company_registration_number',
          'company_fiscal_code',
          'representative_gender',
          'company_representative',
          'company_representative_title',
          'employee_name',
          'employee_country',
          'employee_birth_date',
          'employee_passport_series',
          'employee_passport_number',
          'employee_passport_issued_by',
          'employee_work_permit',
          'employee_work_permit_date',
          'employee_service_address',
          'document_date',
        ],
      });
    } else {
      return NextResponse.json({
        status: 'missing_template',
        message:
          'Template file not found. Please upload adeverinta_WITH_PLACEHOLDERS.docx to public/templates/',
      });
    }
  } catch (error: any) {
    return NextResponse.json(
      {
        status: 'error',
        error: error.message,
      },
      { status: 500 }
    );
  }
}
